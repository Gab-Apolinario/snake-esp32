...
int   tamCobra;
uint16_t corCabeca = VERDE;
uint16_t corCorpo = VERDE_ESCURO;
int   direcao;
...
void desenharScore() {
  ...
  display.setCursor(TELA_X_MIN + 1, TELA_Y_MIN + 1);
  char buf[24];
  sprintf(buf, "SNAKE   SCORE: %d/%d", pontos, pontosVitoria);
  display.print(buf);
  display.drawFastHLine(TELA_X_MIN, 11, 120, CINZA);  //linha separadora
...
void iniciarJogo() {
  ...
  for (int i = 0; i < tamCobra; i++)
    desenharCelula(cobra[i].x, cobra[i].y, corCabeca); //verde
...
void telaGameOver() {
  ...
  display.fillRect(AREA_X, AREA_Y, 127, 127, VERMELHO);
